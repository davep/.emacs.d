...
;;; Supported Emacs:
;;; ================
;;; This program has only been tested on Emacs 20.7.2. If you
...
(defconst muttrc-command-alist
  '(("alias"			((alias) (address)) t nil)
    ...
    ("macro"			((map) (key) (sequence)) nil nil)
...
(defconst muttrc-variables-alist
  '(("abort_nosubject" quadoption "ask-yes")
    ...
    ("alias_format" string "%4n %t %-10a   %r")
...
(let ((command-alist muttrc-command-alist))
  ;; There is no closure in emacs lisp. So a search function is
  ;; defined for all commands with eval
  (while command-alist
    (let ((cmd (caar command-alist)))
      (eval
       (list
	'setq (intern (format "muttrc-search-%s-forward" cmd)))))
...
"
  ...
  (setq font-lock-defaults '('muttrc-font-lock-keywords))
...
(defun muttrc-quote-string (s)
  ...
  (let ((quote-string (char-to-string muttrc-quote-char)))
    (cond ((not s) (format "%s%s" quote-string quote-string))
	  ((string-match "^\\(\\|.*\\s-.*\\)$" s)
	   (concat quote-string
		   (mapconcat (lambda (c)
				(if (eq c muttrc-quote-char)
				    (concat "\\" quote-string)
				  (char-to-string c)))
			      s "")
		   quote-string))
