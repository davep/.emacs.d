...
;;; Copyright (C) 2002, 2003 Matthew P. Hodges

;; Author: Matthew P. Hodges <matt@tc.bham.ac.uk>
;; Version: $Id: eperiodic.el,v 1.59 2003/10/18 18:04:10 matt Exp $
...
(defgroup eperiodic nil
  ...
  :group 'tools
  :link '(url-link "http://www.tc.bham.ac.uk/~matt/EperiodicEl.html"))
...
(defcustom eperiodic-ignored-properties nil
  "List of properties not used by `eperiodic-insert-element-info'."
  :group 'eperiodic
  :type '(repeat symbol))
...
(defcustom eperiodic-property-increment 10
  "Default increment when changing some property.
Associated with some entries in `eperiodic-colour-element-functions'
This can alter `eperiodic-current-temperature' or
...
(defvar eperiodic-last-displayed-element nil
  "The Z value for the element last displayed.
Used to avoid redisplaying the same information.")

(defvar eperiodic-after-display-element-info-hook nil
  "Hook run after element data is redisplayed.

For example:

  (add-hook 'eperiodic-after-display-element-info-hook
...
This is used by `eperiodic-colour-element-by-state'.")
...
(defface eperiodic-discovered-in-face
  '((((class color) (background light))
     (:inherit eperiodic-generic-block-face :background "red" :foreground "black"))
    (((class color) (background dark))
     (:inherit eperiodic-generic-block-face :background "red" :foreground "white")))
...
(defconst eperiodic-element-properties
  '((1
    ...
    (110
     ...
     (discovered-by . "Heavy Ion Research Laboratory (HIRL)")
     (named-after . "Un (one) nun (one) nilium (zero)"))
    (111
     (name . "Unununium")
     (symbol . "Uuu")
     (atomic-mass . "[272]")
...
(if eperiodic-xemacs-p
    (progn
      (defalias 'eperiodic-line-beginning-position 'point-at-bol)
      (defalias 'eperiodic-line-end-position 'point-at-eol))
  (defalias 'eperiodic-line-beginning-position 'line-beginning-position)
  (defalias 'eperiodic-line-end-position 'line-end-position))
...
(defun eperiodic ()
  ...
  (let ((display-width (max eperiodic-element-display-width 3))
    ...
    (while order
      ...
      (let ((orbital-list (car order))
            (width 0)
            face max-z min-z orbital period z)
        ;; Period number
           ...
           (min-z
            (loop for z from min-z to max-z by 1
                  ...
                  (let ((help (if eperiodic-use-popup-help
                    ...
                    (if (fboundp eperiodic-colour-element-function)
  ...
  ;; Make sure first element get displayed
  (setq eperiodic-last-displayed-element nil))

;; Functions

(defun eperiodic-display-element-info ()
  "Display data for the element with atomic number Z."
  (let ((z (eperiodic-element-at)))
    (when (and z (not (equal z eperiodic-last-displayed-element)))
      (save-excursion
      ...
      (run-hooks 'eperiodic-after-display-element-info-hook))))
...
Either the element name or symbol can be used."
  (interactive)
...
(defun eperiodic-next-element (&optional arg)
...
(defun eperiodic-previous-element (&optional arg)
    ...
    (setq url
          (cond
           ((fboundp 'replace-regexp-in-string)
            (replace-regexp-in-string token string url))
           ((fboundp 'replace-in-string)
            (replace-in-string url token string))
           ((and (require 'dired)
                 (fboundp 'dired-replace-in-string))
            (dired-replace-in-string token string url))
           (t
            nil)))
    (if url
        (browse-url url)
...
(defun eperiodic-show-element-properties ()
  ...
  (let ((completion-ignore-case t)
        ...
        name propvalue unit z)
    (setq prop (cdr (assoc (completing-read
                            "Choose property: " list nil nil) list)))
    (when
...
symbol 'key', in which case insert key elements."
  (cond
   ((eq arg 'key)
    ...
    (insert (format " [T = %6.2f K]" eperiodic-current-temperature)))
   (t
  ...
  (cond
   ((eq arg 'key)
    (eperiodic-insert-key-elements '(eperiodic-discovered-before-face
                                     ...
                                     eperiodic-unknown-face))
    (insert (format " [Year = %4d]" eperiodic-current-year)))
   (t
...
(defun eperiodic-insert-key ()
  "Show current key for *Eperiodic* buffer."
  (when (fboundp eperiodic-colour-element-function)
    (insert "\nKey:")
    (add-text-properties (eperiodic-line-beginning-position)
                         (point)
                         `(face eperiodic-header-face))
    (setq face (funcall eperiodic-colour-element-function 'key))
    (insert "\n")))

(defun eperiodic-increase-property (&optional arg)
  "Increase some property by ARG.
  ...
  (cond
   ((eq eperiodic-colour-element-function
    ...
    (setq eperiodic-current-temperature
          (max (+ eperiodic-current-temperature
                  (or arg eperiodic-property-increment)) 0)))
   ((eq eperiodic-colour-element-function
        'eperiodic-colour-element-by-discovery-date)
    (setq eperiodic-current-year
          (+ eperiodic-current-year
             (or arg eperiodic-property-increment)))))
  (let ((posn (point)))
    (eperiodic)
    (eperiodic-display-element-info)
    (goto-char posn)))

(defun eperiodic-decrease-property (&optional arg)
...
(defun eperiodic-cycle-view ()
  "Cycle through elements of `eperiodic-colour-element-functions'."
  (interactive)
...
(defvar eperiodic-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "+")   'eperiodic-increase-property)
    (define-key map (kbd "-")   'eperiodic-decrease-property)
    (define-key map (kbd ">")   'eperiodic-cycle-view)
...
(when (fboundp 'easy-menu-define)
  ...
  (easy-menu-define eperiodic-menu eperiodic-mode-map "Eperiodic Menu"
    '("Eperiodic"
      ...
      "---"
      ["Cycle View"              eperiodic-cycle-view t]
      ("Alter Property"
       ["Increase Temperature"    eperiodic-increase-property 
        (eq eperiodic-colour-element-function 'eperiodic-colour-element-by-state)]
       ["Decrease Temperature"    eperiodic-decrease-property
        (eq eperiodic-colour-element-function 'eperiodic-colour-element-by-state)]
       "---"
       ["Increase Year"           eperiodic-increase-property 
        (eq eperiodic-colour-element-function 'eperiodic-colour-element-by-discovery-date)]
       ["Decrease Year"           eperiodic-decrease-property
        (eq eperiodic-colour-element-function 'eperiodic-colour-element-by-discovery-date)])
      "---"
...
\\{eperiodic-mode-map}"
  ...
  (add-hook 'post-command-hook 'eperiodic-display-element-info t t)
